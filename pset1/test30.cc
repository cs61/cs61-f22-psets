...
#include <cstring>
#include <random>
// Stress test large numbers of allocation sites.

const char* const files[200] = {
...
int main() {
    ...
    // do 10000 allocations from different apparent allocation sites
    for (unsigned i = 0; i != 10000; ++i) {
        const char* file = files[uniform_int(0, nptrs - 1, randomness)];
        int line = 1 + uniform_int(0, nptrs - 1, randomness);
        void* ptr = m61_malloc(1 + uniform_int(0, 127, randomness), file, line);

        int slot = uniform_int(0, nptrs - 1, randomness);
