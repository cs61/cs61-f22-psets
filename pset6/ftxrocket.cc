...
// Usage: ./ftxrocket [-j NTHREADS] [-n NOPS] [FILE]
//    Perform NOPS * NTHREADS “bank transfers” within FILE, completely
//    legally.
...
static void sbf_transfer_thread(ftx_db& db, size_t nops, size_t& opcount,
                                unsigned seed) {
    // Obtain a source of random account numbers
    std::default_random_engine randomness(seed);
    std::uniform_int_distribution pick_sbf_account(size_t(0), size_t(2));
    std::uniform_int_distribution pick_customer_account(size_t(3), db.naccounts - 1);
    std::uniform_int_distribution pick_transfer_type(0, 99);
    std::normal_distribution pick_amount(1000.0, 100.0);

    size_t i = 0;
    while (i != nops) {
        bool internal_transfer = pick_transfer_type(randomness) < 90;

        // Pick two random accounts for transfer
        size_t aindex[2];
        if (internal_transfer) {
            // 90% of the time, move money between our accounts
            aindex[0] = pick_sbf_account(randomness);
            do {
                aindex[1] = pick_sbf_account(randomness);
            } while (aindex[1] == aindex[0]);
        } else {
            // 10% of the time, “borrow” some customer money
            aindex[0] = pick_customer_account(randomness);
            aindex[1] = pick_sbf_account(randomness);
        }
...
int main(int argc, char* argv[]) {
    // Parse arguments
    io61_args args = io61_args("i:D:j:J:n:").set_nthreads(4)
        .set_noperations(100'000)
        .set_ndistinguished_threads(1)
        .parse(argc, argv);
    ...
    for (int i = 0; i != args.nthreads; ++i) {
        if (i < args.ndistinguished_threads) {
            th[i] = std::thread(sbf_transfer_thread, std::ref(*db),
